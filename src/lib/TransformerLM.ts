  ...
  private initializeTransformerLayers(existing?: {
    ...
    for (let i = 0; i < numLayers; i++) {
      const renormState: RMSNormState =
        existing?.renormStates?.[i] ?? ({ gamma: new Array(modelDim).fill(1), epsilon: 1e-6 } satisfies RMSNormState);
      ...
      this.ffWeights1.push(existing?.ff1?.[i] ?? this.initWeightMatrix([modelDim, ffHiddenDim * 2]));
